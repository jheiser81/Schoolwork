...
int main()
{
	...
	addNodeToStartofList(5);
	addNodeToStartofList(4);
	...
	addNodeToEndofList(4);
	addNodeToEndofList(5);
	...
	deleteNodeFromEnd(); //working. Deletes 5 from the end of the list
	...
	deleteSpecificNodeFromList(5); //working. Deletes 5 from the list

	deleteNodeAfterSpecificItemInList(2); //working. Deletes the node after 2 (88)
...
void deleteSpecificNodeFromList(int data)
{
	Node* prevToDelete = NULL; //We do this so we can access the previous node of the node in the list
	Node* toDelete = start;
	bool found = false;

	while (toDelete != NULL)
	{
		if (toDelete->data == data) //if we find the node we want to dleete, break out of the loop
		{
			found = true;
			break;
		}
		prevToDelete = toDelete; //store the previous node so we can use it's pointer in the future
		toDelete = toDelete->next; //move to the next node
	}
	if (!found)
	{
		printf("Could not find node to delete\n");//if we can't find the node we want to delete, exit
		return;
	}
	if (toDelete == start) //if the node we are looking for is the first node in the list
	{
		start = start->next;
		free(toDelete);
		return;
	}
	prevToDelete->next = prevToDelete->next->next; //once we find the node we want to delete, we take our previous node and point past it to break the node we want to delete from the chain.
	free(toDelete); //free the node we want to delete
}
...
void deleteNodeAfterSpecificItemInList(int data)
{
	...
	{
		if (nodeToDelete->next->data == data)
		{
