...
private:
	...
	int totalFetches = 0;
public:
	PetOwner(const std::string& catName, const std::string& dogName, const std::string& catSound, const std::string& dogSound); //had to make 4 parameters to make it work, otherwise was trying to pass in too may arguments and was getting an error

	Animal& getCat(); //had to add & to make it work, otherwise was getting an error
	Animal& getDog();

	void playFetchWithDog(const std::string& item);
	void hearAnimalSound(Animal& animal);
};
